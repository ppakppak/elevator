...
## 통합 대시보드 API (dashboard_server.py)

`dashboard_server.py` 실행 시 제공되는 주요 API:

```bash
python3 dashboard_server.py --host 0.0.0.0 --port 7000
```
...
```bash
# 기본 최신 이벤트
curl -s "http://localhost:7000/api/events?limit=20"

# since(하위호환) 증분 조회
curl -s "http://localhost:7000/api/events?since=120"

# 조건 필터 조회
curl -s "http://localhost:7000/api/events?channel=rtsp&type=fall&severity=critical&min_score=0.85&sort=desc"
```
...
```bash
curl -s "http://localhost:7000/api/events/stats?from_ts=1710000000&to_ts=1710086400"
```

반환: `by_type`, `by_channel`, `by_severity`
...
반환: `uptime_sec`, `event_queue_size`, `db_enabled`, `last_event_ts`, `retry_queue_size`, `token_enabled`

### 4) 접근제어(선택)

환경변수 `DASHBOARD_ACCESS_TOKEN` 설정 시 `/`, `/api/*` 모두 토큰 검증 적용:

```bash
export DASHBOARD_ACCESS_TOKEN="your-secret-token"
...
curl -H "Authorization: Bearer your-secret-token" \
  "http://localhost:7000/api/events?limit=10"
```
